...
const InventoryScreen = ({navigation}: any): React.JSX.Element => {
  ...
  const fetchData = async () => {
    try {
      ...
      const inventoryList = inventorySnapshot.docs.map(
  ...
  const handleShowBottomSheet = () => {
    setIsVisible(true); // Toggle visibility when the button is pressed
  };
  ...
  return (
    <View style={styles.container}>
      <View style={styles.headerContainer}>
        ...
        <CustomButton title="Add Product" onPress={handleShowBottomSheet} />
      </View>
      {loading ? (
        <LoadingDots />
      ) : (
